...
void main()
{
  ...
  sum += TEX2D(xy + vec2(-2.0 * oney, 0.0)) * vec4(c4);
  sum += TEX2D(xy + vec2(-1.5 * oney, 0.0)) * vec4(c3);
  sum += TEX2D(xy + vec2(-1.0 * oney, 0.0)) * vec4(c2);
  sum += TEX2D(xy + vec2(-0.5 * oney, 0.0)) * vec4(c1);
  sum += TEX2D(xy);
  sum += TEX2D(xy + vec2(0.5 * oney, 0.0)) * vec4(c1);
  sum += TEX2D(xy + vec2(1.0 * oney, 0.0)) * vec4(c2);
  sum += TEX2D(xy + vec2(1.5 * oney, 0.0)) * vec4(c3);
